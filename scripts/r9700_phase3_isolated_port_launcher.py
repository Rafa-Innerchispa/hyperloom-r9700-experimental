...
import datetime as dt,hashlib,json,pathlib,socket,subprocess,time
...
PORT=18011
NAME='hyperloom-r9700-p3-int4-repack-p18011'
OLD_NAMES=('hyperloom-r9700-p3-int4-repack','hyperloom-r9700-p3-int4-repack-p8011',NAME)
...
# Fail before touching containers if benchmark port is already occupied.
s=socket.socket()
try:s.bind(('127.0.0.1',PORT))
except OSError as e:raise RuntimeError(f'benchmark port {PORT} is not free: {e}')
finally:s.close()
...
for n in OLD_NAMES: run(['docker','rm','-f',n],t=30)
mounts=[]
