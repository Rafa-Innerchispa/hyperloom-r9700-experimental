...
ROOT=Path(__file__).resolve().parents[1]; MODEL='QuantTrio/Qwen3-Coder-30B-A3B-Instruct-AWQ'; PORT=18011; BASE=f'http://127.0.0.1:{PORT}'
PROMPT='Implement a Python LRU cache with get and put methods. Return only code.'; ROUNDS=8
def one(prompt):
...
def sample_gpu(stop,rows):
 while not stop.is_set():
  ...
  rows.append({'t':time.time(),'card0':card}); stop.wait(0.35)
def c4(round_no):
 ...
 try:
  with concurrent.futures.ThreadPoolExecutor(max_workers=4) as ex:rows=[f.result() for f in [ex.submit(one,PROMPT+f'\nCase {i}:') for i in range(4)]]
...
svc=subprocess.run(['systemctl','--user','is-active','inneros-vllm-canary-rocm10.service'],text=True,capture_output=True).stdout.strip(); rounds=[]
for i in range(ROUNDS):rounds.append(c4(i+1)); time.sleep(1.5)
vals=[r['aggregate_tok_s'] for r in rounds]; hashes=[[x['sha256'] for x in r['rows']] for r in rounds]
out={'schema':'hyperloom.r9700.phase3.c4_stability.v2','captured_at_utc':dt.datetime.now(dt.timezone.utc).isoformat(),'port':PORT,'round_count':ROUNDS,'stock_service_state':svc,'rounds':rounds,'aggregate':{'median_tok_s':statistics.median(vals),'min_tok_s':min(vals),'max_tok_s':max(vals),'range_over_median_pct':(max(vals)-min(vals))/statistics.median(vals)*100,'unique_hash_vectors':len({tuple(x) for x in hashes})}}
