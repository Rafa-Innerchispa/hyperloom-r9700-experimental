    ...
    with urllib.request.urlopen(request, timeout=240) as resp:
        ...
        for raw in resp:
            ...
            if obj.get("choices"):
                ...
                if text and first is None: first = time.monotonic()
                chunks.append(text)
    end = time.monotonic(); rendered = "".join(chunks)
    ct = usage.get("completion_tokens") or max(1, len(rendered.split()))
    return {"ok": status == 200, "status": status, "elapsed_sec": end-t0, "ttft_sec": None if first is None else first-t0, "completion_tokens": ct, "prompt_tokens": usage.get("prompt_tokens"), "decode_tok_s": None if first is None else ct/max(end-first, 1e-9), "text_sha256": hashlib.sha256(rendered.encode()).hexdigest(), "text_len": len(rendered)}


def c4(prompt: str) -> dict:
    t0 = time.monotonic()
    with concurrent.futures.ThreadPoolExecutor(max_workers=4) as ex:
        rows = [f.result() for f in [ex.submit(req, prompt + f"\nCase {i}:", 128) for i in range(4)]]
    wall = time.monotonic()-t0; hashes = [r["text_sha256"] for r in rows]; total = sum(r["completion_tokens"] for r in rows)
    return {"wall_sec": wall, "completion_tokens": total, "aggregate_tok_s": total/max(wall, 1e-9), "rows": rows, "hashes": hashes, "canonical_hashes_match": hashes == CANONICAL_C4}
...
def long_prompt(marker: str) -> str:
...
if out["health"]["ok"]:
    ...
    out["c4"] = c4(short)
    out["long_fresh_y"] = req(long_prompt("y"), 64)
    out["long_fresh_z"] = req(long_prompt("z"), 64)
    out["gate"] = {
        "correctness": bool(out["correctness"]["canonical_match"]),
        "c4_hashes": bool(out["c4"]["canonical_hashes_match"]),
        "c1_floor_60": out["c1"]["decode_tok_s"] >= 60.0,
        "c4_floor_180": out["c4"]["aggregate_tok_s"] >= 180.0,
        "fresh_long_floor_55": min(out["long_fresh_y"]["decode_tok_s"], out["long_fresh_z"]["decode_tok_s"]) >= 55.0,
        "hot_ttft_under_500ms": out["correctness"]["ttft_sec"] is not None and out["correctness"]["ttft_sec"] < 0.5,
    }
...
path = ev / f"r9700_phase5_canary_measure_{captured.strftime('%Y%m%dT%H%M%SZ')}.json"
path.write_text(json.dumps(out, indent=2, sort_keys=True) + "\n")
print(path)
print(json.dumps({k: out.get(k) for k in ("correctness","c1","c4","long_fresh_y","long_fresh_z","gate","pass")}, indent=2, sort_keys=True))
