...
def fixture_arm(concurrency, tokens=10):
        ...
        rounds.append(
            {
                ...
                "total_tokens": tokens * 6 + 30,
                "output_tok_s": tokens * 1.5,
                "total_tok_s": tokens * 1.5 + 7.5,
                "mean_e2e_ms": 350.0,
                "p95_e2e_ms": 575.0,
            }
        )
    return {
        ...
        "aggregate": {
            ...
            "passed": 18,
            "failed": 0,
            "median_output_tok_s": tokens * 1.5,
            "mean_output_tok_s": tokens * 1.5,
            "min_output_tok_s": tokens * 1.5,
            "max_output_tok_s": tokens * 1.5,
            "median_total_tok_s": tokens * 1.5 + 7.5,
            "median_mean_e2e_ms": 350.0,
            "median_p95_e2e_ms": 575.0,
        },
    }
...
def fixture_report():
    return {
        ...
        "failure": None,
        "timestamp_utc": "2026-09-06T10:00:00+00:00",
        "finished_at_utc": "2026-09-06T10:01:00+00:00",
        "runner_source_sha256_start": "a" * 64,
        "runner_source_sha256_end": "a" * 64,
        "hardware_attested_by_runner": False,
        "shell_exposed": False,
        "cdna_specific_paths_used": False,
        "agent_backend": "local-openai",
        "tool_mode": "json",
        "model": "synthetic-not-a-real-model",
        "execution_scope": "unattested_runtime",
        "measurement_rounds": 3,
        "requests_per_round": 6,
        "baseline_concurrency": 1,
        "candidate_concurrency": 2,
        "allowed_candidates": [1, 2],
        "baseline": fixture_arm(1),
        "candidate": fixture_arm(2, tokens=20),
        "agent": {
            "selected_by_model": True,
            "hardcoded_candidate": False,
            "selected_concurrency": 2,
            "progress_log": ["tool: write_file"],
            "decision_context": "15.0000 tok/s. Choose [1, 2].",
        },
        "gate": {
            "gain_fraction": 1.0,
            "gain_percent": 100.0,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (("hardware_attested_by_runner",), True),
        (("shell_exposed",), True),
        (("cdna_specific_paths_used",), True),
        (("execution_scope",), "claimed_amd_worktree_pass"),
        (("runner_source_sha256_end",), "b" * 64),
        (("runner_source_sha256_start",), "invalid"),
        ...
        (("agent", "hardcoded_candidate"), True),
        (("agent", "progress_log"), []),
        (("agent", "selected_concurrency"), 1),
        (("agent", "decision_context"), "999.0000 tok/s"),
        (("gate", "max_p95_ratio"), 2.0),
        (("gate", "min_gain_fraction"), 0.0),
        (("gate", "gain_percent"), 999.0),
        (("verdict",), "REJECT"),
    ],
)
...
def test_runner_retains_every_sample_index(monkeypatch):
    target_spec = importlib.util.spec_from_file_location(
        "r9700_capture_test_target", SCRIPTS / "r9700_upstream_agent_e2e.py"
    )
