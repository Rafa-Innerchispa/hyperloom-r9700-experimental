...
def req(prompt,max_tokens=96):
 ...
 r=urllib.request.Request(BASE+'/v1/completions',data=json.dumps(payload).encode(),headers={'Content-Type':'application/json'})
...
def health():
 try:
  with urllib.request.urlopen(BASE+'/v1/models',timeout=10) as r:return {'ok':r.status==200,'status':r.status,'body':r.read().decode()[:1000]}
...
p=ROOT/'docs'/'evidence'/f"r9700_phase3_isolated_measure_{label}_{dt.datetime.now(dt.timezone.utc).strftime('%Y%m%dT%H%M%SZ')}.json"; p.write_text(json.dumps(out,indent=2,sort_keys=True)+'\n'); print(str(p)); print(json.dumps({k:out.get(k) for k in ('health','correctness','c1','c4','long_context')},indent=2,sort_keys=True))
