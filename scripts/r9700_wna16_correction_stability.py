#!/usr/bin/env python3
"""Fixed-config stability validation for the R9700 precomputed-correction WNA16 kernel.

Freezes the winning config from the bounded tuning run instead of selecting the
best result again. Uses alternating paired HIP-event timing rounds against the
same routed BF16 pre-dequantized vLLM-style reference. No service mutation.
"""
...
ROOT=Path(__file__).resolve().parents[1];SCHEMA='hyperloom.r9700.wna16_correction_stability.v1'
...
def k(A,B,C,S,R,sids,eids,npad,N:tl.constexpr,K:tl.constexpr,EM,nvalid,sam,sak,sbe,sbk,sbn,scm,scn,sse,ssk,ssn,sre,srk,srn,topk:tl.constexpr,BM:tl.constexpr,BN:tl.constexpr,BK:tl.constexpr,GK:tl.constexpr):
 ...
 for g in tl.range(0,K,GK):
  ...
  for sub in tl.static_range(0,GK,BK):
   kk=g+sub+ok;a=tl.load(A+(tok[:,None]//topk)*sam+kk[None,:]*sak,mask=tm[:,None]&(kk[None,:]<K),other=0.).to(tl.bfloat16);pb=tl.load(B+exp*sbe+(kk[:,None]//2)*sbk+on[None,:]*sbn,mask=(kk[:,None]<K)&nm[None,:],other=0);q=((pb>>((kk[:,None]%2)*4))&15).to(tl.bfloat16);qacc=tl.dot(a,q,acc=qacc);asum+=tl.sum(a.to(tl.float32),axis=1)
...
def main():
 try:
  ...
  for M in (1,2,4,8,16):
   ...
   ratios=[p['speedup_x'] for p in pairs];ams=[p['candidate_ms'] for p in pairs];bms=[p['bf16_ms'] for p in pairs];rows.append({'M':M,'candidate_median_ms':float(statistics.median(ams)),'bf16_median_ms':float(statistics.median(bms)),'paired_speedup_median_x':float(statistics.median(ratios)),'paired_speedup_min_x':float(min(ratios)),'paired_speedup_max_x':float(max(ratios)),'wins':sum(1 for x in ratios if x>1.0),'wins_ge_1p05x':sum(1 for x in ratios if x>=1.05),'rounds':len(ratios),'cosine':cos,'max_abs_error':float(diff.max()),'allclose':bool(torch.allclose(Ca.float(),Cb.float(),rtol=.08,atol=.08)),'pairs':pairs})
  out={'pass':all(r['allclose'] and r['cosine']>.999 for r in rows),'device':torch.cuda.get_device_name(0),'torch':torch.__version__,'hip':getattr(torch.version,'hip',None),'config':{'BM':16,'BN':128,'BK':32,'num_warps':4,'num_stages':1,'waves_per_eu':4,'matrix_instr_nonkdim':None},'rows':rows,'M1_gate':{'median_ge_1p05x':rows[0]['paired_speedup_median_x']>=1.05,'wins_ge_15_of_21':rows[0]['wins']>=15,'passes':rows[0]['paired_speedup_median_x']>=1.05 and rows[0]['wins']>=15},'truth_boundary':'fixed config selected by prior run; 21 alternating paired HIP-event rounds; synthetic routed Qwen3 W1; no vLLM service patch'}
...
def main()->int:
 ...
 payload['probe_sha256']=digest(payload);stamp=datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ');p=ROOT/'docs'/'evidence'/f'r9700_wna16_correction_stability_{stamp}.json';p.write_text(json.dumps(payload,indent=2,sort_keys=True)+'\n');print(json.dumps({'pass':payload.get('pass'),'output':str(p),'probe_sha256':payload['probe_sha256'],'M1_gate':(payload.get('probe') or {}).get('M1_gate'),'rows':(payload.get('probe') or {}).get('rows')},sort_keys=True));return 0 if payload.get('pass') else 2
